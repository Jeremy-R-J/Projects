...
function addHonorific(honorific, fullNames){
    ...
    for(i=0; i<fullNames.length;i++){
        ...
        var nameString=honorific+splitName[1]+" "+splitName[0]
        returnArray.push(nameString)
    }
    return returnArray;
}
const hon2 = "Sir";
...
/* 
    ...
    compareVersionNumbers(test2V1, test2V2) should also return 1, because 7.5.2.4 is an earlier
    version than 7.5.3
*/
function compareVersionNumbers(version1, version2){
    ...
    for(i=0; i<version1Split.length;i++){
        if(parseInt(version1Split[i])<parseInt(version2Split[i])){
            return -1;
        }else if(parseInt(version1Split[i])>parseInt(version2Split[i])){
            return 1;
        }
    }
    return -1;
