// * 
//   Given an array of strings
//   return a sum to represent how many times each array item is found (a frequency table)
//   Useful methods:
//     Object.hasOwnProperty("keyName")
//       - returns true or false if the object has the key or not
//     Python: key in dict
// */

const arr1 = ["a", "a", "a"];
// const expected1 = {
//   a: 3,
// };

/* function(arr1)
arr = arr1
newarr = []
   for i in range(arr1.length){
     if arr[i]= 
     newarr.push()
     return 
   }
*/



const arr2 = ["a", "b", "a", "c", "B", "c", "c", "d"];
const expected2 = {
    a: 2,
    b: 1,
    c: 3,
    B: 1,
    d: 1,
};





const arr3 = [];
const expected3 = {};

/**
 * Builds a frequency table based on the given array.
 * - Time: O(?).
 * - Space: O(?).
 * @param {Array<string>} arr
 * @return {Object<string, number>} A frequency table where the keys are items
 *    from the given arr and the values are the amnt of times that item occurs.
 */

/*
  define new object
  iterate through the array and each value as a key with value 1
  use a for key in object loop and add 1 to value each time there is an instance of the key
*/

function frequencyTableBuilder(arr) {
    var obj = {}; // create an object
    for (var i = 0; i < arr.length; i++) { //go through arr
        if (obj[arr[i]] >= 1) { // if key exists, add 1 to its value
            obj[arr[i]]++;
        } else {
            obj[arr[i]] = 1; // if key does not exist, assign value 1
        }
    }
    return obj;
}

console.log(frequencyTableBuilder(arr1));
console.log(frequencyTableBuilder(arr2));
console.log(frequencyTableBuilder(arr3));

module.exports = { frequencyTableBuilder };

/* 
Algo #2:
  Reverse Word Order
  Given a string of words (with spaces)
  return a new string with words in reverse sequence.
*/

const str1 = "This is a test";
const expected1 = "test a is This";

/**
 * Reverses the order of the words but 
 * not the words themselves form the given
 * string of space separated words.
 * - Time: O(?).
 * - Space: O(?).
 * @param {string} wordsStr A string containing space separated words.
 * @return {string} The given string with the word order reversed but the words
 *    themselves are not reversed.
 */
function reverseWordOrder(wordsStr) {}

module.exports = { reverseWordOrder };

/*************************************************************/


// * 
//   Given an array of strings
//   return a sum to represent how many times each array item is found (a frequency table)
//   Useful methods:
//     Object.hasOwnProperty("keyName")
//       - returns true or false if the object has the key or not
//     Python: key in dict
// */

const arr1 = ["a", "a", "a"];
// const expected1 = {
//   a: 3,
// };

/* function(arr1)
arr = arr1
newarr = []
   for i in range(arr1.length){
     if arr[i]= 
     newarr.push()
     return 
   }
*/



const arr2 = ["a", "b", "a", "c", "B", "c", "c", "d"];
const expected2 = {
    a: 2,
    b: 1,
    c: 3,
    B: 1,
    d: 1,
};





const arr3 = [];
const expected3 = {};

/**
 * Builds a frequency table based on the given array.
 * - Time: O(?).
 * - Space: O(?).
 * @param {Array<string>} arr
 * @return {Object<string, number>} A frequency table where the keys are items
 *    from the given arr and the values are the amnt of times that item occurs.
 */

/*
  define new object
  iterate through the array and each value as a key with value 1
  use a for key in object loop and add 1 to value each time there is an instance of the key
*/

function frequencyTableBuilder(arr) {
    var obj = {}; // create an object
    for (var i = 0; i < arr.length; i++) { //go through arr
        if (obj.hasOwnProperty(arr[i])) { // if key (ex. obj['a']) exists, add 1 to its value 
            obj[arr[i]]++;
        } else {
            obj[arr[i]] = 1; // if key does not exist, assign value 1
        }
    }
    return obj;
}

console.log(frequencyTableBuilder(arr1));
console.log(frequencyTableBuilder(arr2));
console.log(frequencyTableBuilder(arr3));

module.exports = { frequencyTableBuilder };

/* 
Algo #2:
  Reverse Word Order
  Given a string of words (with spaces)
  return a new string with words in reverse sequence.
*/

const str1 = "This is a test";
const expected1 = "test a is This";

/**
 * Reverses the order of the words but 
 * not the words themselves form the given
 * string of space separated words.
 * - Time: O(?).
 * - Space: O(?).
 * @param {string} wordsStr A string containing space separated words.
 * @return {string} The given string with the word order reversed but the words
 *    themselves are not reversed.
 */
function reverseWordOrder(wordsStr) {
    var arr2 = ""; /*create a new array*/
    for (var i = wordsStr.length - 1; i >= 0; i--)

}

module.exports = { reverseWordOrder };

/*************************************************************/